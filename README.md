...
| 官网首页 | `airshow.com.cn` | 除了头条 / 轮播，还盯上面的**「门票购买」按钮状态**（见下一节）——这是最硬的信号 |
...
>
> 不过**首页上的「门票购买」按钮**弥补了这一点——按钮一旦从弹窗占位变成指向 `piao.airshow.com.cn` 的真实链接，就等于官网自己宣布开售了。

---

## 四、怎么判定「真的开售了」

程序不是简单搜关键词，那样会被官方辟谣文案骗到。它做四层判断，**从下往上一层比一层硬**：

### 第 1 层（最硬）· 官网「门票购买」按钮的状态

这是**确定性信号**，不需要猜语义，也不会被否定句式骗到——因为它是网站自己说的。

官网首页（`www.airshow.com.cn`）上「门票购买」按钮有两种形态：

| 状态 | 页面源码 |
| --- | --- |
| ⛔ 未开售 | `<a onclick="alert('注册尚未开放，敬请关注!')">门票购买</a>` |
| ✅ 已开售 | `<a href="https://piao.airshow.com.cn">门票购买</a>` |

**按钮从「点了弹窗」变成「带真实链接」，就是开售了。** 程序每轮记录这两个按钮的状态，一旦从占位变成可点，立刻推最高级别告警，标题是「🚨 珠海航展门票购买按钮已开放！」。

> ⚠️ 一个坑：官网源码里还**留着一份被 `<!-- -->` 注释掉的「开售后」写法**（开发者提前备好的）。
> 程序会先把 HTML 注释剥掉再判断，否则上线第一天就会误报「已开售」。
...
官网每条公告都有独立的 `/Item/xxxxx.aspx` 地址。程序记录已见过的公告标题，一旦出现新的、且标题里带「门票 / 票务 / 购票 / 售票 / 票价 / 预售」的公告，按最高优先级告警。

但这里区分两种文案，避免你对最高级告警脱敏：

| 标题命中 | 推送标题 |
| --- | --- |
| 含「开售 / 售票 / 预售 / 销售 / 发售」 | `🚨 珠海航展门票开售信号！` |
| 只含「门票 / 购票 / 票务 / 票价」 | `⚠️ 珠海航展：发现票务相关新内容` |
...
首次运行只建立基线、不发任何通知；日期和时间会被归一化（`2026-09-17` → `<DATE>`），所以页面上的日期刷新不会造成误报。

> 唯一的例外：**如果首次运行时「门票购买」按钮已经是可点状态，仍然会立刻告警**——这是确定性信号，不该因为「第一次运行」就被吞掉。
...
```jsonc
{
  ...
  "button_watch": [                  // 可选：监控页面上按钮/入口的状态
    {
      "label": "门票购买",           // 按钮文字
      "closed_hint": "尚未开放",     // 未开放时 onclick 里会出现的字样
      "open_url_pattern": "piao\\.airshow\\.com\\.cn|ticket"
                                     // 链接匹配到什么才算“已开放”（不填则任意真实链接都算）
    }
  ],
...
> **`button_watch` 是最值得配的一项。** 它监控的不是文字，而是按钮本身的状态：
> 未开放时按钮是弹窗占位（`onclick="alert(...)"`），开放后换成真实链接（`href="..."`）。
> 这个切换是网站自己给出的，比任何关键词都可靠。
>
> 判定结果有四种：`open`（可点）/ `closed`（未开放）/ `unknown`（有链接但不符合购票特征）/
> `missing`（按钮找不到了）。**只有 `open` 会触发最高级别告警**，其余变化推普通提醒。
>
> 程序会先剥掉 HTML 注释再判断——很多网站会把「以后要用的写法」注释在源码里备着，
> 不剥注释就会把注释当成生效代码，直接误报「已开售」。
